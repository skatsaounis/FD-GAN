...
"""
# https://github.com/KaiyangZhou/deep-person-reid/blob/master/torchreid/models/inceptionv4.py
...
class InceptionV4(nn.Module):
    ...
    def __init__(self, cut_at_pooling=False, num_features=1536, **kwargs):
        ...
        #self.classifier = nn.Linear(1536, num_features)

        self.classifier = nn.Sequential(
            nn.Linear(1536, num_features),
            nn.BatchNorm1d(self.num_features)
        )

        # Final convolution is initialized differently from the rest
        #final_conv = nn.Conv2d(1536, self.num_features, kernel_size=1)  # self.num_classes
        #self.classifier = nn.Sequential(
...
def init_pretrained_weights(model, model_url):
    ...
    # Layers that don't match with pretrained layers in name/size are kept unchanged.
...
def inceptionv4(pretrained=True, **kwargs):
